...
\begin{frame}
    ...
    The \alert{model risk} is the expected loss over all $\x \in \X$:

    \begin{block}{Model risk}
        \vspace{-1em}
        \begin{align*}
            R(f) &:= \E_{\x \sim \X}(\E_{\y \mid \x}(L(\y, \f(\x)))) \\
    ...
    \begin{block}{Approximate model risk / batch loss}
        \vspace{-1em}
        \begin{align*}
            \hat{R}(f) &:= \E_{(\x, \y) \sim \B}(L(\y, \f(\x))) \\
            &= \frac{1}{m} \sum_{i=1}^m L(\y_i, \f(\x_i))
        \end{align*}
    \end{block}
    \pause

    The objective of machine learning training is to minimize this loss over some set of data.
\end{frame}
